cd ../makefiles/
make clean
make poisson=hypre bc=mixed config=production multiphase=true poisson=split-fftw precision=single particles=false -j
#make poisson=hypre bc=mixed config=production multiphase=true poisson=split-fftw precision=single particles=false -j
#make poisson=hypre bc=mixed config=debug multiphase=true particles=true -j
cd -
export OMP_NUM_THREADS=48
../makefiles/simulation -file /cluster/scratch_xp/public/cconti/CubismUP/HerrmannFDSplitSP_2 -CFL 0.5 -LCFL 0.1 -bpdx 2 -bpdy 8 -tend .9 -sim rti -tdump 0.01 -split
