	...
	do
		...
		do
			...
			do
				...
				mkdir ${FOLDER}
				cp ../makefiles/simulation ${FOLDER}
				export OMP_NUM_THREADS=48;bsub -n 48 -W 168:00 -o ${BASENAME} -J ${BASENAME} mpirun -np 32 ${FOLDER}/simulation -file ${FOLDER}/${BASENAME} -CFL 0.1 -bpdx ${BPD} -bpdy ${BPD} -tend 200. -rhoS ${D} -ypos .9 -nu ${N} -sim falling -shape ellipse -semiAxisX ${H} -semiAxisY .025 -angle 0.79 -tdump 0.05
			done
		done
	done
done