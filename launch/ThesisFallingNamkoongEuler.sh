cd ../makefiles
make clean
make config=production poisson=split-fftw bc=mixed particles=false rk2=false precision=single dlm=true -j
cd ../launch
...
for BPD in 64
do
	BASENAME=Namkoong_22032016_DLM${D}_Euler_CFL${CFL}_bpdy${BPD}_R${R}
	FOLDER=/cluster/scratch_xp/public/cconti/CubismUP/Thesis/${BASENAME}
	mkdir ${FOLDER}
	cp ../makefiles/simulation ${FOLDER}
    cp ThesisFallingNamkoongEuler.sh ${FOLDER}
    cd ${FOLDER}
