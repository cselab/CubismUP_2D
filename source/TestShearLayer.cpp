...
TestShearLayer::TestShearLayer(const int argc, const char ** argv, const int bpd) : Test(argc, argv), dtCFL(0), dtFourier(0), time(0), bpd(bpd), bpdRef(32),
#ifndef _THIN_
nu(0.002),
#else
nu(0.0001),
...
void TestShearLayer::run()
{
	...
	while (time<endTime)
	{
		...
		if (abs(time-nextDumpTime) < 10*std::numeric_limits<Real>::epsilon())
		{
			stringstream ss;
		...
		if (abs(time-endTime) < 10*std::numeric_limits<Real>::epsilon())
		{
			stringstream ss;
	...
	if (bpd==32)
	{
		...
		serializedGrid << "SerializedGrid_BPD32.grid";
