...
#ifndef CubismUP_2D_OperatorMultigrid_h
#define CubismUP_2D_OperatorMultigrid_h
				...
				for(int bx=0; bx<Nx; bx++)
				{
					...
					for(int iy=0; iy<FluidBlock::sizeY; ++iy)
						for(int ix=0; ix<FluidBlock::sizeX; ++ix)
						{
							...
							else if (giy==size-1) // dirichlet bc
							{
								values[lidx  ] = -(1./rhoE + 1./rhoW + 1./rhoN + 1./rhoS);//-(rhoE + rhoW + rhoN + rhoS);//
...
public:
	MultigridHypre() : nprocsX(8), nprocsY(4), bSetup(false) {}
	
	~MultigridHypre() {}
	
	void setup(FluidGrid * inputGrid, const int rank=0, const int nprocs=1)
	{
		nranks = nprocs;
		fluidgrid = inputGrid;
		...
		size = fluidgrid->getBlocksPerDimension(0) * FluidBlock::sizeX;
		size2 = size*size;
...
#ifndef _PERIODIC_
		int periodicity[2] = { size,0 };
#else
		int periodicity[2] = { size,size };
#endif
...
public:
	MultigridHypreSplit() : nprocsX(8), nprocsY(4), bSetup(false) {}
